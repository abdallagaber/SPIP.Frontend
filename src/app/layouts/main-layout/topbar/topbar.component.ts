    ...
    <header class="h-16 bg-surface-0 dark:bg-surface-900 border-b border-surface-200 dark:border-surface-700 flex items-center justify-between px-4 lg:px-6 sticky top-0 z-10 w-full">
      ...
      <div class="flex items-center gap-3">
        ...
        @if (hasMultipleDashboards()) {
          <p-button variant="outlined" severity="secondary" size="small" styleClass="hidden md:flex" (onClick)="switchDashboard()">
            Switch Dashboard
          </p-button>
        }
...
export class TopbarComponent {
  ...
  readonly hasMultipleDashboards = computed(() => {
    return this.dashboardSelectionService.availableDashboards().length > 1;
  });
  ...
  switchDashboard(): void {
    this.dashboardSelectionService.clearSelectedDashboard();
